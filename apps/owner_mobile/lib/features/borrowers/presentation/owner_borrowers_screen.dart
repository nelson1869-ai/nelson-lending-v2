...
class OwnerBorrowersScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<OwnerBorrowersScreen> createState() =>
      _OwnerBorrowersScreenState();
}
...
class _OwnerBorrowersScreenState extends ConsumerState<OwnerBorrowersScreen> {
  ...
  Future<List<OwnerBorrowerModel>> _load() => ref
      .read(ownerBorrowersApiClientProvider)
      .listBorrowers(search: _search.text);
          ...
          Padding(
              ...
              child: TextField(
                ...
                decoration: const InputDecoration(
                    labelText: 'Search borrowers',
                    prefixIcon: Icon(Icons.search),
                    border: OutlineInputBorder()),
                onSubmitted: (_) => setState(() => _borrowers = _load()),
              )),
          Expanded(
              child: FutureBuilder<List<OwnerBorrowerModel>>(
            future: _borrowers,
            builder: (context, snapshot) {
              ...
              return RefreshIndicator(
                  onRefresh: () async => setState(() => _borrowers = _load()),
                  child: ListView.builder(
                    itemCount: snapshot.data!.length,
                    itemBuilder: (_, index) {
                      final borrower = snapshot.data![index];
                      return ListTile(
